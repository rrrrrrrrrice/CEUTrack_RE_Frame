    ...
    def _track_sequence(self, tracker, seq, init_info):
        ...
        while not right_template:
            ...
            if gt_bbox[2]*gt_bbox[3] < 1:
                right_template = False #真值边界框的宽度和高度乘积是否小于 1，即边界框的面积是否过小。
        ...
        out = tracker.initialize(image, event_image, init_info, idx=i)
        if out is None:
            out = {}
        ...
        for frame_num, (frame_path, event_img_path) in enumerate(zip(seq.frames[i+1:],  seq.event_img_list[i+1:]), start=1):
        ...
        # for frame_num, (frame_path, frame_event_path, event_img_path) in enumerate(zip(seq.frames[i+1:], seq.event_frames[i+1:], seq.event_img_list[i+1:]), start=1):
            rgb_image = self._read_image(frame_path)
            ...
            out = tracker.track(rgb_image, event_img, info)
            prev_output = OrderedDict(out)
