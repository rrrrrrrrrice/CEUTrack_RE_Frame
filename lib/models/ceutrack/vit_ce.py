...
class VisionTransformerCE(VisionTransformer):
    ...
    def forward_features(self, z, x, event_z, event_x,
        ...
        event_x = self.patch_embed(event_x)
        event_z = self.patch_embed(event_z)
        ...
        x = recover_tokens(x, lens_z_new, lens_x, mode=self.cat_mode) #返回x

        rgb_x = x[:, :lens_x]   # RGB search
        event_x = x[:, lens_x:] #event search
        x = torch.cat([event_x, rgb_x], dim=1)
